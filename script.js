...
function rCal(){
  ...
  for(let d=1;d<=dm;d++){
    ...
    if(ft){
      ...
      if(pStart<=2&&pEnd>=98){
        fermStyle='background:rgba(184,48,48,0.85);';
      } else {
        fermStyle='background:linear-gradient(to bottom,transparent '+pStart+'%,rgba(184,48,48,0.6) '+pStart+'%,rgba(184,48,48,0.6) '+pEnd+'%,transparent '+pEnd+'%);';
      }
    }
    var fermLabel='';
    if(ft){
      ...
      fermLabel='<div class="cal-ferme-label" style="color:rgba(139,31,31,0.9);font-weight:700;">Fermé'+( heureLabel?' '+heureLabel:'')+'</div>';
    }
