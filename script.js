...
function card(r){
  ...
  return '<div class="rachel-card" data-id="'+r.id+'">'+
...
function rRecap(){
  db.ref('rachel_historique').once('value').then(snap=>{
    ...
    const mt=Math.round(tH*TAUX*100)/100;
    ...
    document.getElementById('recap-period').innerHTML='<span class="recap-period-label">📅 '+(last?'Depuis le '+fd(new Date(last).toISOString().split('T')[0]):'Toutes les réservations')+'</span>';
      ...
      he.innerHTML=[...hist].reverse().map((p,i)=>{
        ...
        return '<div class="historique-item" data-idx="'+realIdx+'">'+
          ...
          '<div style="display:flex;align-items:center;gap:8px;">'+
            ...
            '<button class="btn-action edit hist-edit" data-idx="'+realIdx+'" title="Modifier"><svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>'+
            '<button class="btn-action del hist-del" data-idx="'+realIdx+'" title="Supprimer"><svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg></button>'+
          '</div>'+
        '</div>';
      }).join('');
...
document.getElementById('btn-payer-rachel').addEventListener('click',()=>{
  ...
  const entry={timestamp:now,heures:d.tH,montantDu:d.mt,montant:montantPaye,periode:debut+' → '+fin};
  db.ref('rachel_historique').push(entry)
    .then(()=>{
      if(inp)inp.value='';
      const diff=montantPaye-d.mt;
      const msg=diff<0?'Paiement ✓ — '+montantPaye.toFixed(2)+' € (reste '+Math.abs(diff).toFixed(2)+' € dû)':diff>0?'Paiement ✓ — '+montantPaye.toFixed(2)+' € (+'+diff.toFixed(2)+' € avance)':'Paiement ✓ — '+montantPaye.toFixed(2)+' €';
      toast(msg,'success');
      rRecap();
    }).catch(()=>toast('Erreur.','error'));
});
