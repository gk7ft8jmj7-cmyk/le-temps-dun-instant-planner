...
function showFermetureModal(){
  ...
  document.getElementById('ferm-save').addEventListener('click',function(){
    ...
    // Check for reservation overlap (with hours)
    var conflict = R.find(function(r){
      if(!r.date) return false;
      var rDep = r.dateDepart || r.date;
      if(rDep < debut || r.date > fin) return false;
      // Check hour overlap for same-day cases
      var rH = r.heure || '00:00';
      var rHD = r.heureDepart || '23:59';
      // For multi-day reservations overlapping the fermeture period
      if(r.date < debut || rDep > fin) return true; // clear overlap
      // Same day check
      if(r.date === debut && rDep === fin){
        return rH < heureFin && rHD > heureDebut;
      }
      return true;
    });
    if(conflict){
      var msg = 'Impossible — '+x(fn(conflict))+' a une réservation ce jour';
      if(conflict.heure) msg += ' à '+conflict.heure;
      toast(msg+'.','error');
      return;
    }
...
function rCal(){
  ...
  for(let d=1;d<=dm;d++){
    ...
    if(ft){
      ...
      else if(ft.type==='matin') fermStyle='background:linear-gradient(to bottom,#B83030 50%,transparent 50%);';
      else if(ft.type==='soir') fermStyle='background:linear-gradient(to bottom,transparent 50%,#B83030 50%);';
    }
    var fermLabel=ft?'<div class="cal-ferme-label">🔴 '+x(ft.motif||'Fermé')+'</div>':''
