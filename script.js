  ...
  c.innerHTML=list.map(r=>{
    ...
    return '<div class="resa-row'+(r.rachel?' resa-row-rachel':'')+'">'+ 
      '<div class="resa-row-left">'+
        '<div class="resa-row-name">'+x(fn(r))+(r.rachel?' <span style="font-size:11px;color:var(--gold);">👩</span>':'')+'</div>'+
  ...
  return '<div class="rachel-card'+(r.rachel?' rachel-card-highlighted':'')+'" data-id="'+r.id+'">'+
    '<div class="rachel-card-header">'+
      '<div class="rachel-card-client">'+
        '<div class="rachel-client-name">'+x(fn(r))+(r.rachel?' 👩':'')+'</div>'+
...
function rRecap(){
  db.ref('rachel_historique').once('value').then(snap=>{
    ...
    const lastCredit=hist.length?(hist[hist.length-1].credit||0):0;
    ...
    // Deduct any credit from last payment (negative credit = still owed)
    const mt=Math.max(0,Math.round((mtBrut-lastCredit)*100)/100);
    const creditDisplay=lastCredit>0?lastCredit:0;
    const debtDisplay=lastCredit<0?Math.abs(lastCredit):0;
    ...
    document.getElementById('recap-period').innerHTML=
      ...
      (creditDisplay>0?'<span class="credit-badge green">Crédit : +'+creditDisplay.toFixed(2)+' €</span>':'')+ 
      (debtDisplay>0?'<span class="credit-badge red">Reste dû : '+debtDisplay.toFixed(2)+' €</span>':'');
    window._rc={tH,mt,mtBrut,last,prevCredit:lastCredit};
    ...
    if(!hist.length){he.innerHTML='<p style="color:var(--gray-400);font-size:14px;">Aucun paiement enregistré.</p>';}
      ...
      he.innerHTML=[...hist].reverse().map((p,i)=>{
        ...
        return '<div class="historique-item" data-idx="'+realIdx+'">'+
          '<div class="historique-left">'+
            '<div class="historique-date">'+fd(new Date(p.timestamp).toISOString().split('T')[0])+'</div>'+
      ...
      he.querySelectorAll('.hist-cancel').forEach(btn=>{
        btn.addEventListener('click',()=>{
          ...
          db.ref('rachel_historique').once('value').then(snap=>{
            ...
            const sorted=Object.entries(entries).sort((a,b)=>a[1].timestamp-b[1].timestamp);
            if(sorted[idx]){
              // Mark as cancelled (keep record but zero the credit)
              db.ref('rachel_historique/'+sorted[idx][0]).update({annule:true,credit:0})
                .then(()=>{toast('Paiement annulé — montant remis dans le dû.','');rRecap();})
                .catch(()=>toast('Erreur.','error'));
            }
          });
        });
