...
function rRecap(){
  db.ref('rachel_historique').once('value').then(snap=>{
    const hist=snap.val()?Object.values(snap.val()).sort((a,b)=>a.timestamp-b.timestamp):[];
    // Active = non-annulled payments
    const activeHist=hist.filter(h=>!h.annule);
    const last=activeHist.length?activeHist[activeHist.length-1].timestamp:null;
    // Credits from active payments (positive = avance paid, reduces future debt)
    const creditFromActive=activeHist.reduce((acc,h)=>acc+(h.credit||0),0);
    // Annulled payments add back their montantDu as debt (credit was zeroed)
    const debtFromAnnulled=hist.filter(h=>h.annule).reduce((acc,h)=>acc+(h.montantDu||h.montant||0),0);
    const totalCredit=creditFromActive-debtFromAnnulled;
    const rr=R.filter(r=>r.rachel&&r.tempsTravail);
    const p=last?rr.filter(r=>r.createdAt>last):rr;
    let tH=0;p.forEach(r=>{tH+=ph(r.tempsTravail);});
    const mtBrut=Math.round(tH*TAUX*100)/100;
    // Net amount due = heures dues - credit restant
    const netDu=Math.round((mtBrut-totalCredit)*100)/100;
    const mt=Math.max(0,netDu);
    const creditDisplay=totalCredit>0?totalCredit:0;
    const debtDisplay=totalCredit<0?Math.abs(totalCredit):0;
    document.getElementById('recap-total-heures').textContent=fh(tH);
    document.getElementById('recap-total-montant').textContent=mt.toFixed(2)+' €';
    document.getElementById('recap-period').innerHTML=
      ...
      (creditDisplay>0?'<span class="credit-badge green">✓ Avance : +'+creditDisplay.toFixed(2)+' €</span>':'')+ 
      (debtDisplay>0?'<span class="credit-badge red">⚠ Reste dû : -'+debtDisplay.toFixed(2)+' €</span>':'');
    // Show debt in montant dû
    document.getElementById('recap-total-montant').textContent=netDu.toFixed(2)+' €';
    document.getElementById('recap-total-montant').style.color=netDu<0?'var(--red)':netDu===0?'var(--green)':'inherit';
    window._rc={tH,mt:Math.abs(netDu),mtBrut,last,prevCredit:totalCredit,netDu};
    const det=document.getElementById('recap-detail');
    if(!p.length){det.innerHTML='<p style="color:var(--gray-400);font-size:14px;padding:16px 0;">Aucune réservation avec temps de travail.</p>';}
    else{det.innerHTML='<table class="recap-table"><thead><tr><th>Client</th><th>Date</th><th>Formule</th><th>Temps</th><th>Montant</th></tr></thead><tbody>'+p.map(r=>{const h=ph(r.tempsTravail),m=Math.round(h*TAUX*100)/100;return'<tr><td>'+x(fn(r))+'</td><td>'+fd(r.date)+'</td><td>'+x(r.formule)+'</td><td>'+x(r.tempsTravail)+'</td><td>'+m.toFixed(2)+' €</td></tr>';}).join('')+'</tbody></table>';}
    const he=document.getElementById('historique-list');
    if(!hist.length){he.innerHTML='<p style="color:var(--gray-400);font-size:14px;">Aucun paiement enregistré.</p>';}
    else{
      he.innerHTML=[...hist].reverse().map((p,i)=>{
        const realIdx=hist.length-1-i; // index in original array
        return '<div class="historique-item'+(p.annule?' historique-annule':'')+' " data-idx="'+realIdx+'">'+
          '<div class="historique-left">'+
            '<div class="historique-date">'+fd(new Date(p.timestamp).toISOString().split('T')[0])+(p.annule?' <span style="color:var(--red);font-size:11px;">ANNULÉ</span>':'')+'</div>'+
            '<div class="historique-detail">'+fh(p.heures)+' · '+p.periode+'</div>'+
          '</div>'+
          '<div style="display:flex;align-items:center;gap:8px;">'+
            '<div class="historique-montant">'+p.montant.toFixed(2)+' €</div>'+
            '<button class="btn-action edit hist-edit" data-idx="'+realIdx+'" title="Modifier montant/date"><svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>'+
            '<button class="btn-action hist-cancel" data-idx="'+realIdx+'" title="Annuler — remet dans montant dû" style="color:var(--amber)"><svg viewBox="0 0 24 24"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg></button>'+
            '<button class="btn-action del hist-del" data-idx="'+realIdx+'" title="Supprimer — efface tout"><svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg></button>'+
          '</div>'+
        '</div>';
      }).join('');
      ...
      he.querySelectorAll('.hist-edit').forEach(btn=>{
        btn.addEventListener('click',()=>{
          ...
          const p=hist[idx];
          const newMontant=prompt('Modifier le montant (€) :', p.montant.toFixed(2));
          if(newMontant===null)return;
          const m=parseFloat(newMontant.replace(',','.'));
          if(isNaN(m)){toast('Montant invalide.','error');return;}
          const newDate=prompt('Modifier la date (JJ/MM/AAAA) :', fd(new Date(p.timestamp).toISOString().split('T')[0]));
          if(newDate===null)return;
          const parts=newDate.split('/');
          let newTs=p.timestamp;
          if(parts.length===3){
            const d=new Date(parts[2]+'-'+parts[1]+'-'+parts[0]);
            if(!isNaN(d.getTime()))newTs=d.getTime();
          }
          // Find firebase key and update
          db.ref('rachel_historique').once('value').then(snap=>{
            const entries=snap.val();if(!entries)return;
            const sorted=Object.entries(entries).sort((a,b)=>a[1].timestamp-b[1].timestamp);
            if(sorted[idx]){
              const key=sorted[idx][0];
              db.ref('rachel_historique/'+key).update({montant:m,timestamp:newTs})
                .then(()=>{toast('Paiement modifié ✓','success');rRecap();})
      ...
      // Bind cancel (remet dans montant dû)
      he.querySelectorAll('.hist-cancel').forEach(btn=>{
        btn.addEventListener('click',()=>{
          if(!confirm('Annuler ce paiement ? Le montant reviendra dans le montant dû. Le paiement restera visible dans l historique.'))return;
          const idx=parseInt(btn.dataset.idx);
          db.ref('rachel_historique').once('value').then(snap=>{
            const entries=snap.val();if(!entries)return;
            const sorted=Object.entries(entries).sort((a,b)=>a[1].timestamp-b[1].timestamp);
            if(sorted[idx]){
              db.ref('rachel_historique/'+sorted[idx][0]).update({annule:true,credit:0,montant:0})
      ...
      he.querySelectorAll('.hist-del').forEach(btn=>{
        btn.addEventListener('click',()=>{
          ...
          db.ref('rachel_historique').once('value').then(snap=>{
            ...
            const entry=sorted[idx][1];
            const wasAnnule=!!entry.annule;
            const msg=wasAnnule?
              'Supprimer ce paiement annulé ? Le montant sera aussi déduit du total dû.':
              'Supprimer définitivement ce paiement ? Il disparaîtra sans affecter le montant dû.';
            if(!confirm(msg))return;
            db.ref('rachel_historique/'+sorted[idx][0]).remove()
              .then(()=>{toast(wasAnnule?'Supprimé — montant déduit du dû.':'Paiement supprimé.','');rRecap();})
              .catch(()=>toast('Erreur.','error'));
          });
        });
      });
...
document.getElementById('btn-payer-rachel').addEventListener('click',()=>{
  const d=window._rc||{};if(!d.tH||d.tH<=0){toast('Aucune heure à payer.','error');return;}
  // If netDu negative (credit), still allow marking period as settled
  const now=Date.now();
  const debut=d.last?fd(new Date(d.last).toISOString().split('T')[0]):'Début';
  const fin=fd(new Date(now).toISOString().split('T')[0]);
  // Custom amount or calculated
  const inp=document.getElementById('recap-montant-paye');
  const customVal=inp?inp.value.trim():'';
  const montantPaye=customVal?parseFloat(customVal.replace(',','.'))||d.mt:d.mt;
  // credit = montantPaye - montantDu (positive = avance, negative = reste dû)
  const credit=montantPaye-d.mt+(d.prevCredit||0);
  const entry={timestamp:now,heures:d.tH,montantDu:d.mt,montant:montantPaye,credit:credit,periode:debut+' → '+fin};
  db.ref('rachel_historique').push(entry)
    .then(()=>{
      if(inp)inp.value='';
      const msg=credit<0?'Paiement ✓ — '+montantPaye.toFixed(2)+' € (reste '+Math.abs(credit).toFixed(2)+' € dû)':credit>0?'Paiement ✓ — '+montantPaye.toFixed(2)+' € (crédit : '+credit.toFixed(2)+' €)':'Paiement ✓ — '+montantPaye.toFixed(2)+' €';
      toast(msg,'success');
      rRecap();
    }).catch(()=>toast('Erreur.','error'));
});
