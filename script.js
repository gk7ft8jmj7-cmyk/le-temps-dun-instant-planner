...
function rRecap(){
  db.ref('rachel_historique').once('value').then(function(snap){
    var raw = snap.val() || {};
    var keys = Object.keys(raw).sort(function(a,b){return raw[a].timestamp-raw[b].timestamp;});
    var hist = keys.map(function(k){return Object.assign({},raw[k],{_key:k});});

    // Dernier paiement non annulé = début période courante
    var activePays = hist.filter(function(h){return !h.annule;});
    var last = activePays.length ? activePays[activePays.length-1].timestamp : null;
    var lastDate = last ? fd(new Date(last).toISOString().split('T')[0]) : null;

    // Heures Rachel depuis dernier paiement actif
    var rr = R.filter(function(r){return r.rachel && r.tempsTravail;});
    var period = last ? rr.filter(function(r){return r.createdAt > last;}) : rr;
    var tH = 0;
    period.forEach(function(r){tH += ph(r.tempsTravail);});
    var heuresDues = Math.round(tH * TAUX * 100) / 100;

    // Paiements annulés dans période courante → leur montant reste dû
    var annulesDansPeriode = hist.filter(function(h){
      return h.annule && (!last || h.timestamp > last);
    });
    var montantAnnules = annulesDansPeriode.reduce(function(acc,h){
      return acc + (h.montantOriginal || 0);
    }, 0);

    var totalDu = Math.round((heuresDues + montantAnnules) * 100) / 100;
    ...
    // Période
    var firstResa = period.length ? period.reduce(function(a,b){return a.createdAt<b.createdAt?a:b;}) : null;
    var periodeText = '';
    if(firstResa && lastDate) periodeText = 'Du '+fd(new Date(firstResa.createdAt).toISOString().split('T')[0])+' au '+lastDate;
    else if(firstResa) periodeText = 'Depuis le '+fd(new Date(firstResa.createdAt).toISOString().split('T')[0]);
    else periodeText = 'Toutes les réservations';
    ...
    if(!hist.length){
      ...
      he.innerHTML = hist.slice().reverse().map(function(p){
        var mo = p.montantOriginal || 0;
        var mp = p.montant || 0;
        var montantHTML = p.annule ?
          '<span style="text-decoration:line-through;color:var(--gray-400);font-size:14px;">'+mo.toFixed(2)+' €</span>' :
          (Math.abs(mo-mp)>0.01 ?
            '<span style="text-decoration:line-through;color:var(--gray-400);font-size:12px;">'+mo.toFixed(2)+' €</span> <span class="historique-montant" style="color:var(--green);">'+mp.toFixed(2)+' €</span>' :
        ...
        return '<div class="historique-item'+(p.annule?' historique-annule':'')+'" data-key="'+p._key+'">'+
          ...
          '<div style="display:flex;align-items:center;gap:8px;">'+
            '<div style="text-align:right;">'+montantHTML+'</div>'+
            (!p.annule ? '<button class="btn-action hist-cancel" data-key="'+p._key+'" data-montant="'+mp+'" title="Annuler" style="color:var(--amber);"><svg viewBox="0 0 24 24"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg></button>' : '')+
            '<button class="btn-action del hist-del" data-key="'+p._key+'" data-annule="'+(p.annule?'1':'0')+'" data-montant="'+mp+'" title="Supprimer"><svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg></button>'+
          '</div>'+
        '</div>';
      }).join('');
      ...
      he.querySelectorAll('.hist-cancel').forEach(function(btn){
        btn.addEventListener('click', function(){
          ...
          db.ref('rachel_historique/'+key).update({annule:true, montantOriginal:montant})
            .then(function(){toast('Paiement annulé — montant remis dans le dû.','success');rRecap();})
      ...
      he.querySelectorAll('.hist-del').forEach(function(btn){
        btn.addEventListener('click', function(){
          var wasAnnule=btn.dataset.annule==='1';
          var msg=wasAnnule?
            'Supprimer ce paiement annulé ? Son montant sera aussi retiré du total dû.' :
            'Supprimer définitivement ? Il disparaîtra sans affecter le montant dû.';
          if(!confirm(msg))return;
          db.ref('rachel_historique/'+btn.dataset.key).remove()
            .then(function(){toast('Supprimé.','');rRecap();})
            .catch(function(){toast('Erreur.','error');});
        });
      });
    }

    window._rc = {tH:tH, mt:totalDu, netDu:totalDu, last:last};
  });
}
...
document.getElementById('btn-payer-rachel').addEventListener('click', function(){
  ...
  var now = Date.now();
  var last = d.last;
  var debut = last ? fd(new Date(last).toISOString().split('T')[0]) : 'Début';
  ...
  db.ref('rachel_historique').push(entry).then(function(){
    ...
    var msg = diff < -0.01 ? 'Payé '+montantPaye.toFixed(2)+' € (reste '+Math.abs(diff).toFixed(2)+' € dû)' :
              diff > 0.01  ? 'Payé '+montantPaye.toFixed(2)+' € (avance '+diff.toFixed(2)+' €)' :
