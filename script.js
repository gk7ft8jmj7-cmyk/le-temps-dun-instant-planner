...
function rRecap(){
  db.ref('rachel_historique').once('value').then(function(snap){
    ...
    // Total payé = somme des paiements NON annulés
    // Annulés = comme si pas payé (montant revient dans dû)
    var totalPaye = hist.filter(function(h){return !h.annule;})
    ...
    var lastActivePay = hist.filter(function(h){return !h.annule;});
    ...
    if(!hist.length){
      ...
      // Supprimer — efface complètement, le montant ne compte plus nulle part
      he.querySelectorAll('.hist-del').forEach(function(btn){
        btn.addEventListener('click', function(){
          if(!confirm('Supprimer définitivement ce paiement ? Il disparaîtra complètement.'))return;
